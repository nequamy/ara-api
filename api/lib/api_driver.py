...
logging.basicConfig(level=logging.INFO, filename='ara_mini.log', filemode='w')
...
class MultirotorControl:
    ...
    def __enter__(self):
        self.is_transmitter_open = not self.connect(trials=self.ser_trials)

        if self.is_transmitter_open:
            return self
        else:
            return 1
    ...
    def fast_read_altitude(self):
        ...
        if self.send_RAW_msg(MultirotorControl.MSPCodes['MSP_SONAR']):
            data_length = 4
            msg = self.receive_raw_msg(size=(6 + data_length))[5:]
            converted_msg = struct.unpack('<i', msg[:-1])[0]
            self.SENSOR_DATA['sonar'] = round((converted_msg / 100.0), 2)  # correct scale factor
    ...
    def fast_read_rc_channels(self):
        if self.send_RAW_msg(MultirotorControl.MSPCodes['MSP_RC']):
            ...
            self.process_MSP_RC(msg)

    def fast_read_debug(self):
        if self.send_RAW_msg(MultirotorControl.MSPCodes['MSP_DEBUG']):
            data_length = 8
            msg = self.receive_raw_msg(size=(6 + data_length))[5:]
            # print(msg)
            converted_msg = struct.unpack('<%dh' % (data_length / 2), msg[:-1])
            # print(converted_msg)
            self.SENSOR_DATA['debug'] = converted_msg

            # print(self.SENSOR_DATA['debug'])
    ...
    def receive_raw_msg(self, size, timeout=10):
        with self.serial_port_read_lock:  # It's necessary to lock everything because order is important
            local_read = self.transmitter.serial_client.read
