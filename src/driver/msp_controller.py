...
from driver.msp_transmitter import Transmitter
...
class MultirotorControl:
    ...
    def __init__(self, transmitter: Transmitter, logger):
        ...
        with self.serial_port_read_lock:  # It's necessary to lock everything because order is important
            ...
            while True:
                try:
                    ...
                    di += 1
                    self.logging.debug(
                        "State: {1} - byte received (at {0}): {2}".format(dataHandler['last_received_timestamp'],
                                                                          dataHandler['state'],
                                                                          data))
                except IndexError:
                    self.logging.debug('IndexError detected on state: {}'.format(dataHandler['state']))
                    dataHandler['state'] = -1
                ...
                if dataHandler['state'] == 0:  # sync char 1
                    ...
                    if (data == 77):  # M - followed by an M => MSP V1
                        ...
                        self.logging.debug('Something went wrong, no M received.')
                        break  # sends it to the error state
                elif dataHandler['state'] == 2:  # direction (should be >)
                    dataHandler['unsupported'] = 0
                    if (data == 33):  # !
                        # FC reports unsupported message error
                        self.logging.debug('FC reports unsupported message error.')
                    ...
                    dataHandler['message_buffer_uint8_view'] = dataHandler[
                        'message_buffer']  # keep same names from betaflight-configurator code
                    ...
                    self.logging.debug(
                        "JumboFrame message_length_expected: {}".format(dataHandler['message_length_expected']))
                    ...
                    if dataHandler['msp_version'] == 1:
                        if dataHandler['message_checksum'] == data:
                            ...
                            self.logging.debug("Message received (length {1}) - Code {0}".format(dataHandler['code'],
                                                                                            dataHandler[
                            ...
                            # wrong checksum
                            self.logging.debug(
                                'Code: {0} - crc failed (received {1}, calculated {2})'.format(dataHandler['code'],
                                                                                               data,
                                                                                               dataHandler[
                                                                                                   'message_checksum']))
                        ...
                        dataHandler['message_checksum'] = self._crc8_dvb_s2(dataHandler['message_checksum'],
                                                                            dataHandler['code'] & 0xFF)  # code LOW
                        dataHandler['message_checksum'] = self._crc8_dvb_s2(dataHandler['message_checksum'], (
                                    dataHandler['code'] & 0xFF00) >> 8)  # code HIGH
                        dataHandler['message_checksum'] = self._crc8_dvb_s2(dataHandler['message_checksum'],
                                                                            dataHandler[
                                                                                'message_length_expected'] & 0xFF)  # HIGH
                        dataHandler['message_checksum'] = self._crc8_dvb_s2(dataHandler['message_checksum'], (
                                    dataHandler['message_length_expected'] & 0xFF00) >> 8)  # HIGH
            ...
            self.logging.debug('Error detected on state: {}'.format(dataHandler['state']))
    ...
    def save2eprom(self):
        self.logging.info("Save to EPROM requested")  # some configs also need reboot to be applied (not online).
        return self.send_RAW_msg(MultirotorControl.MSPCodes['MSP_EEPROM_WRITE'], data=[])
    ...
    def set_RX_MAP(self, new_rc_map):
        ...
        return self.send_RAW_msg(MultirotorControl.MSPCodes['MSP_SET_RX_MAP'], new_rc_map)

    def set_FEATURE_CONFIG(self, mask):
        assert (type(mask) == int)

        data = self.convert([mask], 32)
        return self.send_RAW_msg(MultirotorControl.MSPCodes['MSP_SET_FEATURE_CONFIG'], data)
    ...
    def send_RAW_RC(self, data=[]):
        ...
        return self.send_RAW_msg(MultirotorControl.MSPCodes['MSP_SET_RAW_RC'], data)
    ...
    def process_recv_data(self, dataHandler):
        ...
        if code == 0:  # code==0 means nothing was received...
            ...
            if (not dataHandler['unsupported']):
                processor = MultirotorControl.__dict__.get("process_" + MultirotorControl.MSPCodes2Str[code])
                if processor:  # if nothing is found, should be None
                    try:
                        ...
                        self.logging.debug('Received data processing error: {}'.format(err))
                        return -4
                else:
                    self.logging.debug('Unknown code received: {}'.format(code))
                    return -5
            else:
                self.logging.debug('FC reports unsupported message error - Code {}'.format(code))
                return -6
    ...
    def process_MSPV2_INAV_OPTICAL_FLOW(self, data):
        self.SENSOR_DATA['optical_flow'][0] = self.readbytes(data, size=8, unsigned=True)
        self.SENSOR_DATA['optical_flow'][1] = radians(self.readbytes(data, size=16, unsigned=False))
        self.SENSOR_DATA['optical_flow'][2] = radians(self.readbytes(data, size=16, unsigned=False))
        self.SENSOR_DATA['optical_flow'][3] = radians(self.readbytes(data, size=16, unsigned=False))
        self.SENSOR_DATA['optical_flow'][4] = radians(self.readbytes(data, size=16, unsigned=False))
    ...
    def process_MSP_MISC(self, data):  # 22 bytes
        if self.INAV:
            ...
            self.MISC['minthrottle'] = self.MOTOR_CONFIG['minthrottle'] = self.readbytes(data, size=16,
                                                                                         unsigned=True)  # 0-2000
            self.MISC['maxthrottle'] = self.MOTOR_CONFIG['maxthrottle'] = self.readbytes(data, size=16,
                                                                                         unsigned=True)  # 0-2000
            self.MISC['mincommand'] = self.MOTOR_CONFIG['mincommand'] = self.readbytes(data, size=16,
                                                                                       unsigned=True)  # 0-2000
    ...
    def process_MSP_API_VERSION(self, data):
        ...
        self.CONFIG['apiVersion'] = str(self.readbytes(data, size=8, unsigned=True)) + '.' + str(
    ...
    def process_MSP_BOARD_INFO(self, data):
        ...
        if data:
            ...
            for i in range(MultirotorControl.SIGNATURE_LENGTH):
